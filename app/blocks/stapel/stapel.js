import $ from 'jquery';
export const stapel = function (window) {
	...
	$.Stapel.defaults = {
		...
		onAfterOpen(pileName, totalItems) {
			return false;
		},
		onBeforeClose(pileName) {
			return false;
		},
		onAfterClose(pileName, totalItems) {
			return false;
		}
	};
	...
	$.Stapel.prototype = {
				...
				for (var pile in this.piles) {
					...
					for (var i = 0, len = p.elements.length; i < len; ++i) {
						...
						pile === this.pileName ? this._applyTransition($item, fs, this.options.pileAnimation.closeSpeed, function (evt) {
							...
							opacity: 1
						}, this.options.otherPileAnimation.openSpeed, function (evt) {
		...
		_applyTransition(el, styleCSS, speed, fncomplete) {
